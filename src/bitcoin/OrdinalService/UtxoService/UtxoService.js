...
class UtxoService {
    ...
    getUtxos = async (address, confirmations = 2) => {
        try {
            const response = await fetch(`${this.baseUrl}/unspent?active=${address}&confirmations=${confirmations}&cors=true`);
            if (!response.ok) {
            ...
            if (data.unspent_outputs.some((output) => output.confirmations < confirmations)) {
                throw new Error("Please wait for two confirmations on each transaction");
            }
